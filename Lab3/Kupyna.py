import copy

from util import *


class Kupyna:
    # l is in bytes
    _n_to_l = {256: 64, 384: 128, 512: 128}
    _l_to_t = {64: 10, 128: 14}
    _l_to_c = {64: 8, 128: 16}

    _s_box_0 = (
        
    )

    _s_box_1 = (
        
    )

    _s_box_2 = (
        
    )

    _s_box_3 = (
        
    )

    _mds_matrix = [
        [0x01, 0x01, 0x05, 0x01, 0x08, 0x06, 0x07, 0x04],
        [0x04, 0x01, 0x01, 0x05, 0x01, 0x08, 0x06, 0x07],
        [0x07, 0x04, 0x01, 0x01, 0x05, 0x01, 0x08, 0x06],
        [0x06, 0x07, 0x04, 0x01, 0x01, 0x05, 0x01, 0x08],
        [0x08, 0x06, 0x07, 0x04, 0x01, 0x01, 0x05, 0x01],
        [0x01, 0x08, 0x06, 0x07, 0x04, 0x01, 0x01, 0x05],
        [0x05, 0x01, 0x08, 0x06, 0x07, 0x04, 0x01, 0x01],
        [0x01, 0x05, 0x01, 0x08, 0x06, 0x07, 0x04, 0x01]
    ]

    _s_boxes = [_s_box_0, _s_box_1, _s_box_2, _s_box_3]

    def __init__(self, n: int):
        assert n in self._n_to_l
        self.n = n
        self.l = self._n_to_l[n]
        self.t = self._l_to_t[self.l]
        self.c = self._l_to_c[self.l]

        self._mult_mat = Kupyna._gen_mult()

    @staticmethod
    def _gen_mult():
        m = [[0] * 8 for _ in range(256)]
        for i in range(256):
            for j in range(1, 9):
                m[i][j - 1] = Kupyna._multiply_gf(i, j)
        return m

    def _pad(self, msg):
        length = len(msg) * 8
        msg.append(0x80)
        while (len(msg) + 12) % self.l != 0:
            msg.append(0x00)

        msg += length.to_bytes(12, "little")

        assert len(msg) % self.l == 0
        return msg

    def hash(self, msg):
        msg = bytearray(msg)
        padded = self._pad(msg)

        state = Kupyna._bytes_to_mutable_state((1 << 510).to_bytes(self.l, "big"), 8) if self.l == 64 else \
            Kupyna._bytes_to_mutable_state((1 << 1023).to_bytes(self.l, "big"), 8)

        for i in range(0, len(padded), self.l):
            msg_block = padded[i: i + self.l]

            m_v = Kupyna._bytes_to_mutable_state(msg_block, 8)

            first = Kupyna._xor_states(state, m_v)
            self._t1(first)

            second = m_v
            self._t2(second)

            state = Kupyna._xor_states(second, state)
            state = Kupyna._xor_states(state, first)

        prev = copy.deepcopy(state)
        self._t1(prev)
        state = Kupyna._xor_states(prev, state)
        b = Kupyna._state_to_bytes(state)
        return b[-(self.n // 8):]

    def _t1(self, s):
        for v in range(0, self.t):
            Kupyna._add_mod_2(s, v)
            Kupyna._sub_bytes(s)
            Kupyna._shift_rows(s, self.c)
            self._mix_cols(s)

    def _t2(self, s):
        for v in range(0, self.t):
            Kupyna._add_mod_64(s, self.c, v)
            Kupyna._sub_bytes(s)
            Kupyna._shift_rows(s, self.c)
            self._mix_cols(s)

    @staticmethod
    def _add_mod_n(g: bytearray, k: bytes, n: int):
        a = int.from_bytes(g, "little")
        b = int.from_bytes(k, "little")
        return bytearray(((a + b) % n).to_bytes(8, "little"))

    @staticmethod
    def _xor_states(a: list, b: list):
        c = [bytearray(k ^ l for (k, l) in zip(c1, c2)) for (c1, c2) in zip(a, b)]
        assert len(c) == len(a)
        assert len(c[0]) == len(a[0])
        return c

    @staticmethod
    def _add_mod_2(s: list, v: int):
        for j in range(len(s)):
            s[j][0] ^= (j << 4) ^ v

    @staticmethod
    def _add_mod_64(s: list, c: int, v: int):
        for j in range(len(s)):
            s[j] = Kupyna._add_mod_n(s[j],
                                     b"\xf3\xf0\xf0\xf0\xf0\xf0\xf0" + (((c - 1 - j) << 4) ^ v).to_bytes(1, "little"),
                                     two_in_64)

    @staticmethod
    def _sub_bytes(s):
        for col_i in range(len(s)):
            for row_i in range(len(s[col_i])):
                s[col_i][row_i] = Kupyna._s_boxes[row_i % 4][s[col_i][row_i]]

    @staticmethod
    def _shift_rows(s, c):
        for row_i in range(1, 8):
            shift = row_i
            if c == 16 and row_i == 7:
                shift = 11
            shifted_end = list(s[i][row_i] for i in range(c - shift, c))
            for col_i in range(c - 1, shift - 1, -1):
                s[col_i][row_i] = s[col_i - shift][row_i]
            for col_i in range(shift):
                s[col_i][row_i] = shifted_end[col_i]

    def _multiply_matrices(self, m, s):
        result = [bytearray(8) for _ in range(len(s))]
        for col_i in range(len(s)):
            for row_i in reversed(range(8)):
                product = 0
                for b in reversed(range(8)):
                    # product ^= Kupyna._multiply_gf(s[col_i][b], m[row_i][b])
                    product ^= self._mult_mat[s[col_i][b]][m[row_i][b] - 1]
                result[col_i][row_i] = product

        return result

    def _mix_cols(self, s):
        s[:] = self._multiply_matrices(Kupyna._mds_matrix, s)

    @staticmethod
    def _multiply_gf(x, y):
        r = 0
        for i in range(8):
            if (y & 0x1) == 1:
                r ^= x
            hbit = x & 0x80
            x <<= 1
            if hbit == 0x80:
                x ^= 0x011d
            y >>= 1
        return r

    @staticmethod
    def _bytes_to_state(b, rows=8):
        return list(b[i:i + rows] for i in range(0, len(b), rows))

    @staticmethod
    def _bytes_to_mutable_state(b, rows=8):
        return list(bytearray(b[i:i + rows]) for i in range(0, len(b), rows))

    @staticmethod
    def _state_to_bytes(s):
        return b''.join(s)
